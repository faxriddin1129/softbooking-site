<template>
  <div class="dark:bg-black-input config-border-radius bg-white-card p-3 transition-150">
    <h1 class="text-[18px] dark:text-gray-300 transition-150 hidden sm:block">{{$t("105")}}</h1>
...
const getOrders = async function () {
  try {
    const res = await useApi().$get<ICheckUser>("v1/flight/orders-active?include=services");
    data.value = res?.data
    loading.value = false
  } catch (err) {
    ElNotification({
